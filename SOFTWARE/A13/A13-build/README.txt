...
2. Building Uboot
-----------------
Here you have two options. The first is to use sunxi u-boot and the second is to use mainline u-boot. 

The sunxi u-boot is suitable for the boards that have HYNIX DDR3 chip - H5TQ2G83CFR. The mainline u-boot is suitable for boards that have SAMSUNG DDR3 chip - K4B2G0846Q.
...
2.1. Building sunxi u-boot 

Let's download the uboot sources from GitHub repository, note there are lot of branches but you have to use sunxi branch.
...
Download a13_linux_defconfig using wget command 
...
/home/user/a13-olinuxino-kernel_3.4.90# directory 
# mkdir /mnt/sd
# mount /dev/sdX1 /mnt/sd
...
# /home/user/a13-olinuxino-kernel_3.4.90
download debian rootfs - a13_olinuxino_debian_fs_kernel_34_90_rel_10.tgz using wget command
...
Connect USB-SERIAL-CABLE-F to UEXT Tx.Rx and GND, or connect a HDMI screen. Put the SD-card in A13-OLinuXino and apply 6-16V power, you should see Uboot and then Kernel messages on the console
